...
const PhytoSchema = (() => {
  ...
  const TEMPORAL_DIMENSION_LEXICON = /\b(date|dob|year|quarter|day|timestamp|month)\b/i;
  // A subset of the above whose *values*, when numeric, are actually Excel date
  // serials that should be displayed as a readable date rather than a raw
  // number (e.g. a "Month" or "Month format" column holding 45597 instead of
  // text like "Nov") — "year"/"quarter"/"day" are deliberately excluded since
  // those commonly hold plain small numbers (2024, 1-31) that are NOT serials.
  const DATE_SERIAL_HEADER_LEXICON = /\b(date|dob|timestamp|month)\b/i;
  const MONTH_GRANULARITY_LEXICON = /\bmonth\b/i;
  // Excel date serials for real-world business data land roughly here
  // (~1954-2064); guards against misreading a plain small number (a "Day"
  // column's 1-31, a bare "Year" column's 2024) as a date serial.
  const PLAUSIBLE_DATE_SERIAL_MIN = 20000;
  const PLAUSIBLE_DATE_SERIAL_MAX = 60000;
  ...
  // Common trailing "sub-measure" words a period label gets concatenated
  // with in a single-row header (e.g. "April Qty", "April Value") — as
  // opposed to a 2-row merged header, where the period and sub-measure are
  // already in separate cells. See splitConcatenatedPeriodMeasure().
  const MEASURE_SUFFIX_LEXICON = /^(qty|quantity|value|units?|sale|sales|amount|amt|target|rate|cost|revenue|volume|count|total)$/i;
  // How many leading rows to scan past when looking for the real header row
  // — real-world files commonly have several banner/metadata rows (title,
  // "Last Updated", a blank spacer row) before the actual column headers.
  const MAX_LEADING_ROWS_TO_SCAN = 15;
  ...
  /**
   * Splits a single-cell header like "April Qty" or "April Value" into a
   * period ("April") and a sub-measure ("Qty") — the single-row-header
   * equivalent of a 2-row merged header's parent/sub-label split. Without
   * this, a file whose months are written as one word per column (a very
   * common real-world convention) would show up as N unrelated standalone
   * measures instead of one proper time-period series.
   */
  function splitConcatenatedPeriodMeasure(label) {
    const s = cellText(label);
    if (!s) return null;
    ...
    if (MEASURE_SUFFIX_LEXICON.test(last) && matchesTimePeriodLexicon(prefix)) {
      return { period: prefix, measure: last };
    }
    return null;
  }
  ...
  /**
   * Finds where the real header row likely starts, skipping past any
   * leading banner/metadata rows (a title, a "Last Updated" line, blank
   * spacer rows — real-world exports commonly have several of these, not
   * just one). A row counts as "real header material" once it has more than
   * 2 non-blank cells; anything sparser than that is treated as a banner
   * row and skipped. Falls back to row 0 if nothing in the scanned range
   * looks header-shaped (e.g. a very narrow, 1-2 column sheet).
   */
  function findHeaderStartRow(aoa) {
    const maxScan = Math.min(aoa.length, MAX_LEADING_ROWS_TO_SCAN);
    for (let r = 0; r < maxScan; r++) {
      const row = aoa[r] || [];
      const nonBlankCount = row.filter(c => cellText(c) !== '').length;
      if (nonBlankCount > 2) return r;
    }
    return 0;
  }
  ...
  function detectHeaderLayout(aoa, merges) {
    const headerStart = findHeaderStartRow(aoa);

    // Merge signal (authoritative): a horizontal merge spanning >=2 columns
    // on the header-start row means a 2-row merged header starting there.
    const hasHeaderMerge = merges.some(m => m.s.r === headerStart && m.e.r === headerStart && m.e.c > m.s.c);
    if (hasHeaderMerge) {
      return { headerRowCount: 2, dataStartRow: headerStart + 2 };
    }

    // Lexicon fallback (no merge metadata available): the row right below
    // looks like Units/Value-style sub-labels.
    const subRow = aoa[headerStart + 1] || [];
    const nonBlankSub = subRow.filter(c => cellText(c) !== '');
    if (nonBlankSub.length > 0) {
      const matches = nonBlankSub.filter(c => SUBHEADER_LEXICON.test(cellText(c)));
      if (matches.length / nonBlankSub.length >= 0.5) {
        return { headerRowCount: 2, dataStartRow: headerStart + 2 };
      }
    }

    return { headerRowCount: 1, dataStartRow: headerStart + 1 };
  }
   ...
   */
  function classifyColumn(aoa, colIndex, dataStartRow, headerLabel, parentLabel, headerRowCount) {
    ...
    // Single-row-header equivalent of the merged-header parent/sub split —
    // e.g. "April Qty" / "April Value" as one concatenated cell each.
    if (headerRowCount === 1) {
      const split = splitConcatenatedPeriodMeasure(headerLabel);
      if (split) {
        return {
          role: 'time-period', confidence: 'high',
          reason: `Header combines a time period ("${split.period}") with a measure ("${split.measure}")`,
          sampleValues, derivedPeriodLabel: split.period, derivedMeasureLabel: split.measure,
        };
      }
    }
    if (TEMPORAL_DIMENSION_LEXICON.test(normalizedHeader)) {
      // If the values are actually numeric Excel date serials (not already
      // readable text like "Nov"), format them as a real date instead of
      // showing the raw serial number.
      const numericSamples = sampleValues.map(v => PhytoParser.toNumberOrNull(v)).filter(v => v !== null);
      const looksLikeSerial = DATE_SERIAL_HEADER_LEXICON.test(normalizedHeader)
        && numericSamples.length > 0
        && numericSamples.every(v => v >= PLAUSIBLE_DATE_SERIAL_MIN && v <= PLAUSIBLE_DATE_SERIAL_MAX);
      const dateFormat = looksLikeSerial ? (MONTH_GRANULARITY_LEXICON.test(normalizedHeader) ? 'mmm-yy' : 'dd-mmm-yyyy') : null;
      return {
        role: 'dimension', confidence: 'high',
        reason: dateFormat
          ? `Header name suggests a date column; numeric values will be shown as dates (e.g. "${XLSX.SSF.format(dateFormat, numericSamples[0])}")`
          : 'Header name suggests a date/calendar column, not a summable metric',
        sampleValues, dateFormat,
      };
    }
  ...
  function assembleGroups(columns, headerRowCount) {
    ...
    columns.forEach((col, idx) => {
      ...
      if (col.role === 'dimension') {
        dimensions.push({
          colIndex: col.colIndex, label: col.headerLabel, key: makeUniqueKey(col.headerLabel, usedKeys),
      ...
      if (col.role === 'time-period') {
        const groupLabel = col.derivedPeriodLabel
          || (headerRowCount === 2 && col.parentLabel ? col.parentLabel : col.headerLabel);
        if (!timePeriodGroups.has(groupLabel)) {
          timePeriodGroups.set(groupLabel, { label: groupLabel, order: idx, subCols: [] });
        }
        const measureLabel = col.derivedMeasureLabel
          || (headerRowCount === 2 && col.subLabel ? col.subLabel : 'Value');
  ...
  function detectSchema(workbook, sheetName) {
    ...
    const columns = blocks.map(b => ({
      ...b,
      ...classifyColumn(aoa, b.colIndex, dataStartRow, b.headerLabel, b.parentLabel, headerRowCount),
    }));
  ...
  function applyUserEdits(schema, columnRoleOverrides, primaryMeasureKeyOverride, headerLayoutOverride) {
    ...
    if (layoutChanged) {
      ...
      columns = blocks.map(b => ({
        ...b,
        ...classifyColumn(schema._aoa, b.colIndex, dataStartRow, b.headerLabel, b.parentLabel, headerRowCount),
      }));
    }
